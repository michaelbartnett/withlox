...
- [ ] Finish remaining nested value editors

- [ ] Exploratory design for breadcrumb value navigation in-editor

- [ ] Exploratory relational-join for nested arrays

## 2016-11-14-0453EST NO REALLY WHAT NOW OH F---

The dramatic log entry title is because Donald Trump is now
President-elect of the United States. Sigh.

Anyway, I now have the fully recursive value editor window working.

ImGui really shit the bed when trying to do nested column tables. I
suppose that's to be expected. The docs say that columns are not
really ready for prime time.

That's okay though, because I think that a nested table view might
actually just be terrible anyway, based on the half-working versions I
sort of managed to cobble together.

I think a clearer path forward is to effectively do a join of each
nested array element on a uniquely identifying column of the outer
object and field name and index of the array element.

If the top-level value is an array, then we can make a
table. Otherwise everything has to be a tree view thingy.

This is perfectly fine for now, since I think I will change from an
ImGui based UI to something much more custom later on down the
line. Maybe using NanoVG?  To reiterate from long ago: the dream is
for it to feel like Google Sheets. ImGui is just not made for that use
case. It's really awesome to test with for now, though.

There are a couple of remaining nested value cases to work on, and
then I want to try some of the basic breadcrumb and relational-join
style value editing. The breadcrumb thing is basically a way to delve
deeper into an array's structure, and then climb back out. The
relational-join is what I just described, and it might be a bit too
much to do right now. We'll see. I mainly want to see if I can get
multiple views into the data and be able to edit it semi-effectively.

The other thing I can probably do is hoisting compound values that
don't have any intermediate arrays into top-level columns for the
table editor. The tricky part is when two different compound values
don't have the same keys. Right now I just write a `NOPE` label which
isn't terribly clear, but clearly we'll need some sort of `undefined`
indicator. Maybe there can even be an option to insert a
default. That's thinking a bit ahead, though.

With a robust value editing system, then I can better experiment with
types and values again.

Once that feels not terrible, then there are two other thigns on my
mind: re-export and schemas. Re-export is exactly what it sounds like,
and I can probably do a naive "write everything" routine without much
hassle. Later we'll want a dirty flag of some kind.

Then there's type schemas. This is a textual description of a type
represented in the collection. It will start as a type definition
parser and a window to paste those types in. This will make it easier
to prototype tuples and enums, as well as type migrations and
coercions on collections. It will also be a time to start thinking
about named types. Probably before all the other things, since writing
type schemas without named types will probably be a huge PITA. 

This feature may cause the return of the `TypeRef` structure, since
now the type of a component of another type may be rather than a
literal type.  Can values have named type refs? Maybe that's important
so that you can do those wide-ranging value transformations.

Those transformations will be some of the most important details to
think about, so I'm looking forward to making the type schema parser.
...
- [X] Separate loaded record metadata from `Value`s

- [X] Try to write the collection editor to work recursively so that
...
- http://research.microsoft.com/en-us/um/people/simonpj/papers/gadt/gadt-rigid-contexts.pdf
- http://lambda-the-ultimate.org/node/1955
- http://cs.brown.edu/courses/cs173/2012/book/types.html#(part._.Type_.Inference)
- https://www.cs.cornell.edu/courses/cs3110/2011sp/lectures/lec26-type-inference/type-inference.htm
- https://www.typescriptlang.org/docs/handbook/type-inference.html
- http://c2.com/wiki/closed.html?TypeInference
...
I also putzed around with tracing allocator calls. It was somewhat
educational.
